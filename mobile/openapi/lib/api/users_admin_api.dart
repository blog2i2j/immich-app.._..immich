...
class UsersAdminApi {
  ...
  ///
  /// * [bool] isTrashed:
  Future<Response> getUserStatisticsAdminWithHttpInfo(String id, { bool? isArchived, bool? isFavorite, bool? isTrashed, }) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// * [bool] isTrashed:
  Future<AssetStatsResponseDto?> getUserStatisticsAdmin(String id, { bool? isArchived, bool? isFavorite, bool? isTrashed, }) async {
    final response = await getUserStatisticsAdminWithHttpInfo(id,  isArchived: isArchived, isFavorite: isFavorite, isTrashed: isTrashed, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
