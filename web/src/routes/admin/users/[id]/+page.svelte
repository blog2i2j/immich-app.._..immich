    ...
    <Container size="large" center>
      <div class="grid gap-4 grid-cols-2 w-full">
        ...
        <div>
          <Card color="secondary">
            ...
            <CardBody>
              <Stack gap={2}>
                <div>
                  <Heading tag="h3" size="tiny">Name</Heading>
                  <Text>{user.name}</Text>
                </div>
                <div>
                  <Heading tag="h3" size="tiny">Email</Heading>
                  <Text>{user.email}</Text>
                </div>
                <div>
                  <Heading tag="h3" size="tiny">Created</Heading>
                  <Text>{user.createdAt}</Text>
                </div>
                <div>
                  <Heading tag="h3" size="tiny">Updated</Heading>
                  <Text>{user.updatedAt}</Text>
                </div>
                <div>
                  <Heading tag="h3" size="tiny">ID</Heading>
                  <Code>{user.id}</Code>
                </div>
              </Stack>
            </CardBody>
            <CardFooter class="border-0 flex justify-end">
              <Button leadingIcon={mdiPencil} onclick={() => handleSomething} variant="filled" color="primary">
                <Text class="hidden md:block">{$t('edit')}</Text>
              </Button>
            </CardFooter>
          </Card>
        </div>
        <Card color="secondary">
          ...
          <CardHeader>
            <div class="flex items-center gap-2">
              <Icon icon={mdiAccountCogOutline} size="1.5rem" />
              <CardTitle>Preferences</CardTitle>
            </div>
          </CardHeader>
          <CardBody>
            <div>
              <Heading tag="h3" size="tiny">Features</Heading>
